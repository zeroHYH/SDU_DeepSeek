...
def sbox_lookup(m: int, i: int, j: int) -> list[int]:
    """
    使用 S_BOX 查表，返回对应的4位输出（二进制位列表）
    """
    value = S_BOX[m][i][j]
    return [int(b) for b in f"{value:04b}"]

def sBoxPermute(expandByte: list[int]) -> list[int]:
    """S盒置换操作：将48位输入压缩为32位输出"""
    sBoxByte = []
    for m in range(8):
        # 第 m 组的6位
        offset = m * 6
        bits = expandByte[offset:offset+6]

        # 行号 i: 第1位和第6位组成 (2位)
        i = (bits[0] << 1) | bits[5]
        # 列号 j: 中间4位 (bits[1~4])
        j = (bits[1] << 3) | (bits[2] << 2) | (bits[3] << 1) | bits[4]

        # 查表并添加结果
        sBoxByte.extend(sbox_lookup(m, i, j))
